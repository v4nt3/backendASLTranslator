...
app = FastAPI(
    ...
    description=(
        ...
        "Receives keypoint sequences pre-extracted with MediaPipe in the browser "
...
def _validate_keypoints(keypoints: list) -> np.ndarray:
    ...
    if arr.shape[1] != EXPECTED_FEATURE_DIM:
        raise HTTPException(
            status_code=422,
            detail=(
                f"Each frame must have {EXPECTED_FEATURE_DIM} features, "
                f"got {arr.shape[1]}. "
                f"Expected: left_hand(63) + right_hand(63) + face(204) + "
                f"body(99) + velocity(429) = 858"
            ),
        )
    return arr
...
@app.post(
    ...
    tags=["Prediction"],
    summary="Predict a single isolated sign from keypoint frames",
...
@app.get(
    "/labels",
    tags=["System"],
    summary="Get all available sign labels",
...
@app.get(
    "/config",
    tags=["System"],
    summary="Get inference configuration (internal use only)",
    dependencies=[Depends(verify_api_key)],
)
